...
export function generateMockCharacterDescription(input: { childName: string; gender: CharacterGender; age: AgeRangeId }): string {
 ...
 * demonstrates the name/gender → character → consistent-illustrations
 * pipeline. The look is seeded from (childName, theme) so it stays identical
 * across the cover and every page of one story, and changes between
 * different stories.
 */
export function generateMockCharacterPortrait(params: { childName: string; gender: CharacterGender; themeId: string }): string {
  ...
  const figure = renderPortraitFigure({ cx: width / 2, cy: height / 2 - 40, scale: 1.05, look, gender: params.gender });
